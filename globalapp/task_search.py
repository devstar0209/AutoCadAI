...
def build_region_instruction(region: str, unit: str) -> str:
    ...
    return (
        ...
        "Use prefered unit conventions. DON't include another measure units."
    )
...
Categories MUST match exactly with System. For example, if system is Plumbing, category must be a plumbing-related category such as "Plumbing Fixtures", "Domestic Water Distribution", "Sanitary Drainage", etc. Do NOT use general categories like "General Requirements" or "Site Work" for specific systems.
...
def search_material_cost(
    ...
    for system_name, items in system_to_items.items():
        ...
        print("Searching material costs for system:", system_name)
        ...
        for i in range(0, len(items), BATCH_SIZE):
            ...
            for item in batch_items:
                ...
                batch_input.append(obj)
            batch_json = json.dumps(batch_input)

            response = client.responses.create(
                model="gpt-5.2",
                input=f"""
                You are a cost estimation assistant. The user provides a JSON array of construction materials. For each item, search the web for the current material cost per unit in {region}.
                ...
                Use real values from the web search. If any value is unknown, estimate cost according to similar items. Output only the JSON, no extra text.
                """,
                tools=[
                    {
            ...
            res = extract_json_from_response(response.output_text)
            returned_items = json.loads(res)
            # print("Received cost estimation for batch:", i, data[0])
            # returned_items = data[0].get("items", [])

            print(f"Batch {i//BATCH_SIZE + 1}: Retrieved material costs for {len(returned_items)} items.")

            for idx, item in enumerate(batch_items):
                cost = returned_items[idx].get("material_unit_cost", 0)
                item["material_unit_cost"] = cost
                updated_items.append(item)

        system_to_items[system_name] = updated_items

    return system_to_items
# -----------------------------
# Cost Estimate (material costs + labor + equipment)
# -----------------------------
COST_SYSTEM_PROMPT = """
You are a U.S construction cost estimator.
...
{
  "items":[
    {"Item":"...","quantity":number,"unit":"...","unit_labor_rate":number,"unit_equipment_rate":number}
  ]
}
"""
        ...
        for i in range(0, len(items), BATCH_SIZE):
            ...
            for item in batch_items:
                obj = {
                    ...
                    "Item": item.get("Item", ""),
                    "unit": item.get("unit", ""),
                    "labor_hours_per_unit": item.get("labor_hours_per_unit", 0),
                    "equipment_hours_per_unit": item.get("equipment_hours_per_unit", 0)
                }
                batch_input.append(obj)
            batch_json = json.dumps(batch_input)
...
def start_pdf_processing(pdf_path: str, output_excel, output_pdf, location, currency, session_id, cad_title, unit, user_filter_sentence):
    ...
    material_cost_items = search_material_cost(system_to_items, location, currency)
    
    print_step("8) Estimate costs for items (GPT)")
    cost_items = estimate_costs_for_items(material_cost_items, location, currency)

    print_step("9) Export to Excel (one sheet, system name as merged row)")
